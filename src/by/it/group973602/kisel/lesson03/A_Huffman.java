package by.it.group973602.kisel.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        s.chars().forEach(item -> {
            if (count.containsKey((char) item))
                count.put((char) item, count.get((char) item) + 1);
            else
                count.put((char) item, 1);
        });
        ...
        while (priorityQueue.size() > 1) {
            priorityQueue.add(new A_Huffman.InternalNode(priorityQueue.remove(),priorityQueue.remove()));
        }
