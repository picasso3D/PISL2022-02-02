package by.it.group973602.bakhmatov.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (char c : s.toCharArray()) { //количество каждого симвл в строке
            count.put(c, count.getOrDefault(c, 0) + 1);
        }

        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for (Map.Entry<Character, Integer> entry: count.entrySet()) { //приоритетная очередь по уб частоты
            priorityQueue.add(new LeafNode(entry.getValue(), entry.getKey()));
        }
        while (priorityQueue.size() > 1) { //бинарное дерево
        ...
        StringBuilder sb = new StringBuilder(); //проходим по дереву
        assert priorityQueue.peek() != null;
        priorityQueue.peek().fillCodes(sb.toString()); //каждому символу нужный код
        for (char c: s.toCharArray()) { //шифрование строки
